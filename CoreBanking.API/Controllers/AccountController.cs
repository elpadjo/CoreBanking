...
using CoreBanking.Application.Accounts.Queries.GetTransaction;
...
using CoreBanking.Application.Accounts.Queries.GetTransferHistory;
...
public class AccountsController : ControllerBase
{
    ...
    //! FOR TRANFERS HISTORY - TO BE iIMPLEMENTED LATER
    // Get transfer history for an account

    [HttpGet("{accountNumber}/tranfers")]
    public async Task<ActionResult<ApiResponse<TransactionHistoryDto>>> GetTransferHistory(
        string accountNumber,
        [FromQuery] DateTime? startDate = null,
        [FromQuery] DateTime? endDate = null,
        [FromQuery] int page = 1,
        [FromQuery] int pageSize = 50)
    {
        ...
        return Ok(ApiResponse<TransferHistoryDto>.CreateSuccess(result.Data!));
    }

    // Get transfer by id
    [HttpGet("{accountNumber}/transfer/{transferId}")]
    public async Task<ActionResult<ApiResponse<TransferDto>>> GetTransferById(
        string accountNumber,
        Guid transferId)
    {
        ...
        {
            AccountNumber = AccountNumber.Create(accountNumber),
            TransferId = TransferId.Create(transferId)
        };
        ...
        return Ok(ApiResponse<TransferDto>.CreateSuccess(result.Data!));
    }
}