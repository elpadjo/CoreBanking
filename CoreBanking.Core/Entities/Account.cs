...
namespace CoreBanking.Core.Entities
{
    public class Account : AggregateRoot<AccountId>, ISoftDelete
    {
        ...
        public Result Debit(Money amount, string description, string reference)
        {
            ...
            // Update balances by creating new Money objects (avoid using operators to prevent EF Core tracking issues)
            CurrentBalance = new Money(CurrentBalance.Amount - amount.Amount, CurrentBalance.Currency);
            AvailableBalance = new Money(AvailableBalance.Amount - amount.Amount, AvailableBalance.Currency);
            UpdateTimestamp();
